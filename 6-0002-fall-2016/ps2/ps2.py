...
#
# Answer:
# The nodes represent buildings in the MIT campus.
# The edges represent the path between two buildings in the campus.
# The distances between buildings are represented as the weights
# associated with each edge.
# One weight represents the total distance of the path. Another
# represents the distance spent outdoors.
...
def load_map(map_filename):
    ...
    with open(map_filename, "r") as f:
        for line in f:
            ...
            graph.add_edge(WeightedEdge(src, dest, int(parts[2]), int(parts[3])))
...
#
# Answer:
# The objective function is the total distance between the source and the destination.
# We would like to minimize this.
# The constraint is the upper bound on total distance spent outdoors.


def get_distances(digraph, path):
    total_dist = 0
    outdoor_dist = 0
    for node in path:
        for edge in digraph.get_edges_for_node(Node(node)):
            print(edge, edge.get_destination() == Node('4'))
            if str(edge.get_destination()) == node:
...
print(graph)
print(get_distances(graph, ["2", "4", "8"]))
...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist, best_path):
    ...
    for edge in digraph.get_edges_for_node(start_node):
        if edge.get_destination() not in path[0]:
            total_dist, outdoor_dist = get_distances(digraph, path[0])
            print(total_dist, outdoor_dist)
            if best_path == None or (
                outdoor_dist <= max_dist_outdoors and total_dist <= best_dist
            ):
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    if best_path[1] > max_total_dist or best_path[2] > max_dist_outdoors:
        raise ValueError

    return [node.get_name() for node in best_path[0]]
